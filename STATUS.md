...
| Multiplayer 1v1 / 2v2 lockstep | experimental | ✅ N-seat lockstep, full hash + **v0.54: title-menu MULTIPLAYER lobby** (relay/room/mode fields, persisted relay, generated room code, copyable invite link) + **waiting room** (connection status, invite, honest relay-unreachable message — no more blank screen/blocking alert) | ✅ lobby gate + fg7 gate + **5-min contested 2-browser SOAK over the real relay: 5,805 lockstep ticks, both seats build/train/fight, zero desync, exact world agreement at 14 checkpoints** (`MPSOAK=1`, `_mp_soak.spec.ts`). Still pending: a live 2-HUMAN match (operator-side) — now one click via the lobby |
...
| Playtest smoothness pass (v0.53) | RA-smooth openings + honest feedback | Full scripted playtest (title-menu flow, motion sampling, event capture — `PLAYTEST=1`, `tests/liveness/_playtest.spec.ts`) found and fixed: **base alarm now structure-HP-only** (was firing for field skirmishes — cry-wolf), **"Harvester lost." EVA line** (economy death was silent), **empty-selection right-click no longer acks** (no marker, no sound — RA acknowledges only when someone obeys), **harvester force-return** (right-click own refinery with cargo → early dock, RA/D2K micro), **T1 build times on the RA cost line** (barracks 20→14s, power 30→19s; measured 23s→16s), **briefing hint + sidebar legend no longer clip**. Movement already RA-smooth: 9px/300ms, zero stalls. | ✅ +2 unit tests (marker honesty, force-return) + playtest telemetry before/after |

| Placement feel (v0.54) | RA red-grid | **A refused placement click keeps the ghost in hand** (RA behavior: reposition, don't re-press the hotkey) + buzz + EVA "Cannot deploy there" (was: silently consumed the mode AND the sim rejected with zero feedback — found by the MP soak's blind clicks) · ghost validity now checked for the VIEWER's seat (was hardcoded 'player' — wrong ghost for MP seat B) | ✅ eva_sell gate extended (refused click → EVA line + ghost retained) |

| RA operating model (v0.55) | operate like C&C Red Alert | **Structures now build IN THE SIDEBAR** (RA's defining flow, per the official manual + EA's GPL source): click → paid upfront → clock-sweep progress on the button → flashing **READY — click to place** → placement ghost → short ~3s on-field unfold. **One structure per team at a time** (refused clicks say the iconic "Unable to comply — building in progress"). **Right-click the sidebar icon cancels** — structure jobs refund in full (mid-build or READY); unit buttons pop the last queued copy or refund the one in production. Sidebar right-clicks never leak onto the battlefield. Low power halves sidebar construction speed (RA manual rule). The job is authoritative sim state (in stateHash, MP/replay-safe). | ✅ 4 unit tests (`ra_build_flow.test.ts`: upfront charge + one-at-a-time, ready-gated placement + fast unfold + no double charge, cancel refund, hash sensitivity) + 7 gates migrated to the flow + full suite 41/41 |
| Parallel-session integration (v0.55) | — | Phases 1a–2 (24 units / 25 structures / tech-tree / squad AI, from the parallel content session) integrated: fixed a squad-leader election reading a nonexistent `veterancy` component (elections silently never happened), a faction-lock check reading a nonexistent `state.playerFaction` (locks never engaged), and typed the tech-tree fields into the Refinement schema instead of `as any` casts. | ✅ verify 289 tests + 41 gates + balance sweep 9/9 seat-symmetric with the new content |
